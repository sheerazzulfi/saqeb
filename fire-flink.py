...
def login(token):
    ...
    suiteid = 'SUITE1014'
    pes = s.post('http://test.flinko.com:8109/optimize/v1/dashboard/execution/suite/' + suiteid, headers=head)
    ...
    sc = 0
    while (sc < 1):
        r1 = s.get('http://test.flinko.com:8109/optimize/v1/dashboard/execution/' + exid, headers=head)
        c1 = json.loads(r1.content)
        fr1 = c1['responseObject']['executionStatus']
        print('status : ' + fr1 + '......')
        if (fr1 == "Completed" or fr1 == "Terminated" or fr1 == "Warning"):
            if fr1 == 'Completed':
                r2 = s.get('http://test.flinko.com:8110/optimize/v1/executionResponse/result/' + exid, headers=head)
